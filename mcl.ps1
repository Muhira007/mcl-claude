...
$Script:ConfigDir    = Join-Path $env:APPDATA 'mcl'
$Script:ConfigFile   = Join-Path $Script:ConfigDir 'config'

# --- defaults ----------------------------------------------------------------
# Based on official MiMo docs: https://docs.mimo.mi.com/devpack/tool/claude
$Script:DefaultOpusSonnet  = 'mimo-v2.5'
$Script:DefaultHaikuModel  = 'mimo-v2.5'
...
function Write-DebugX {
  param([string]$Msg)
  if ($env:MCL_VERBOSE -eq '1') { Write-Host "[DEBUG] $Msg" -ForegroundColor DarkGray }
}
...
function Save-Key {
  ...
  if (-not (Test-KeyFormat $Key)) {
    Write-Warn 'MiMo keys should follow the format: {API Key ID}.{secret}'
    Write-Warn 'Example: abc123xyz.abcdefghijklmnopqrstuvwxyz'
    Write-Warn 'Saving anyway, but it may not work.'
  }
  Write-Config 'MCL_API_KEY' $Key
  Write-Say "Key saved to $Script:ConfigFile"
}

function Get-Key {
  return Read-Config 'MCL_API_KEY'
}

# --- key validation ----------------------------------------------------------
function Test-KeyFormat {
  param([string]$Key)
  # MiMo keys follow the format: {API Key ID}.{secret}
...
function Test-KeyApi {
  ...
  try {
    ...
    Write-Warn 'Could not reach MiMo API (network error).'
    return $true
  }
}
...
function Invoke-Setup {
  Write-Say ''
  Write-Say '+------------------------------------------+'
  Write-Say '|  mcl - first-time setup                  |'
  Write-Say '+------------------------------------------+'
  Write-Say ''
  Write-Say "Claude Code will run against MiMo's MiMo-5.2 API."
  Write-Say 'You only need to enter your key once.'
  Write-Say ''
  Write-Say '1. Buat akun di https://mimo.mi.com'
  Write-Say '2. Setelah login, buka https://mimo.mi.com/manage-apikey/apikey-list'
  Write-Say ''
  ...
  Write-Say ''
  Write-Say 'WARNING: Keep your key secure. MiMo auto-revokes publicly exposed keys.'
  Write-Say ''
  for ($i = 0; $i -lt 3; $i++) {
    $secure = Read-Host -AsSecureString 'MiMo API key'
...
function Show-Config {
  $key = Get-Key
  $savedSafe = if ($val = Read-Config 'MCL_SAFE') { $val } else { '0' }
  ...
  Write-Host "Safe mode   : $(if ($env:MCL_SAFE) { $env:MCL_SAFE } else { $savedSafe })"
  Write-Host '---'
  Write-Host 'Model overrides (from config or env):'
  $opusSonnet = if ($env:MCL_OPUS_SONNET_MODEL) { $env:MCL_OPUS_SONNET_MODEL } else { $v = Read-Config 'MCL_OPUS_SONNET_MODEL'; if ($v) { $v } else { $Script:DefaultOpusSonnet } }
  $haiku      = if ($env:MCL_HAIKU_MODEL)        { $env:MCL_HAIKU_MODEL }        else { $v = Read-Config 'MCL_HAIKU_MODEL';        if ($v) { $v } else { $Script:DefaultHaikuModel } }
  $timeoutMs  = if ($env:MCL_TIMEOUT_MS)         { $env:MCL_TIMEOUT_MS }         else { $v = Read-Config 'MCL_TIMEOUT_MS';         if ($v) { $v } else { $Script:DefaultTimeoutMs } }
...
function Invoke-DryRun {
  ...
  $key        = Get-Key
  $safeMode   = if ($env:MCL_SAFE)                { $env:MCL_SAFE }                else { $v = Read-Config 'MCL_SAFE';                if ($v) { $v } else { '0' } }
  $opusSonnet = if ($env:MCL_OPUS_SONNET_MODEL)   { $env:MCL_OPUS_SONNET_MODEL }   else { $v = Read-Config 'MCL_OPUS_SONNET_MODEL';   if ($v) { $v } else { $Script:DefaultOpusSonnet } }
  $haiku      = if ($env:MCL_HAIKU_MODEL)          { $env:MCL_HAIKU_MODEL }          else { $v = Read-Config 'MCL_HAIKU_MODEL';          if ($v) { $v } else { $Script:DefaultHaikuModel } }
  $timeoutMs  = if ($env:MCL_TIMEOUT_MS)           { $env:MCL_TIMEOUT_MS }           else { $v = Read-Config 'MCL_TIMEOUT_MS';           if ($v) { $v } else { $Script:DefaultTimeoutMs } }
  $autoCompact = if ($env:MCL_AUTO_COMPACT)         { $env:MCL_AUTO_COMPACT }         else { $v = Read-Config 'MCL_AUTO_COMPACT';         if ($v) { $v } else { $Script:DefaultAutoCompact } }

  Write-Host '═══════════════════════════════════════════════'
  Write-Host '  mcl dry-run'
  Write-Host '═══════════════════════════════════════════════'
  Write-Host ''
  ...
  Write-Host '      MiMo-5.2 maps: low/med/high->high, xhigh/max/ultracode->max'
  Write-Host ''
  if ($safeMode -eq '1') {
...
function Invoke-Subcommand {
  ...
  if ($Cmd -match '^(config|--config|set-key|--set-key|change|--change|change-key|--change-key)$') {
    ...
    Write-Say "Done. Run 'mcl' to start."
    exit 0
    ...
    Write-Config 'MCL_OPUS_SONNET_MODEL' $model
    ...
    $key = Get-Key
    if (-not $key) { Write-ErrorX "No stored key. Run 'mcl config' first." }
    ...
    if ($key) {
      ...
      if (Test-KeyApi $key) {
        Write-Say "[v] API Key can reach MiMo servers successfully."
      } else {
        Write-Warn "[x] API Key failed validation."
        $healthy = $false
      }
    } else {
      Write-Warn "[x] No API Key set. Run 'mcl config'."
      $healthy = $false
    }
    
    if ($healthy) {
      Write-Say "`nSystem is fully ready to use Mcl!"
    } else {
    ...
    $aliasCmd = "Set-Alias $aliasName mcl"
    Add-Content -Path $PROFILE -Value "`n# Added by mcl`n$aliasCmd"
    Write-Say "[v] Alias '$aliasName' for 'mcl' has been added to your PowerShell profile ($PROFILE)."
...
while ($i -lt $args.Count) {
  switch ($args[$i]) {
    ...
    'help'      { Show-Help }
    '-h'        { Show-Help }
    '--version' { Write-Host "mcl v$Script:Version"; exit 0 }
    '--dry-run' { $dryRun = $true; $i++ }
    '--verbose' { $env:MCL_VERBOSE = '1'; $i++ }
    '--safe'    { $env:MCL_SAFE = '1'; $i++ }
...
Write-DebugX "mcl v$Script:Version starting"
    ...
    if ($check) {
      ...
      if ($resp.StatusCode -eq 200) {
        ...
        if ($remoteVer -ne $Script:Version -and $remoteVer -match '^\d+\.\d+\.\d+') {
          Write-Host "`n🚀 New version of Mcl is available ($remoteVer)! Run 'mcl update' to install.`n" -ForegroundColor Yellow
        }
      }
...
Write-DebugX "MCL_SAFE=$(if ($env:MCL_SAFE) { $env:MCL_SAFE } else { '0' })"
Write-DebugX "MCL_VERBOSE=$(if ($env:MCL_VERBOSE) { $env:MCL_VERBOSE } else { '0' })"

# --- resolve the key ---------------------------------------------------------
$key = Get-Key
Write-DebugX "Key from config: $(if ($key) { "found ($($key.Length) chars)" } else { 'not found' })"

if (-not $key -and $env:MCL_API_KEY) {
  $key = $env:MCL_API_KEY.Trim()
  Write-Say 'Using MCL_API_KEY from environment; saving for next time.'
  try {
    Save-Key $key
  } catch {
...
if (-not $key) {
  Write-ErrorX "No API key available. Run 'mcl config' to set one."
}
...
if (-not (Get-Command claude -ErrorAction SilentlyContinue)) {
  ...
  $ans = Read-Host "Would you like Mcl to automatically install Claude Code via npm? [Y/n]"
  if ($ans -notmatch "^[nN]") {
